...
gamestate = None
win_status = ""
opening_round_filled = [False, False, False, False, False, False] 

@app.route("/")
def index():
    return render_template("index.html", gamestate=gamestate, win_status=win_status)
...
def handle_place_card(data):
    ...
    if player == gamestate.getTurn() and gamestate.numturns > 5 and (placed_card_value1 != 11) and (placed_card_value1 != 13):
        ...
        if player == "1" and hand_index < len(gamestate._hand1.getHand()) and placed_card_value1 < 11:
            if opening_round_filled[caravan_index] == False:
                opening_round_filled[caravan_index] = True
                gamestate.placeCard(player, hand_index, caravan_index)
                gamestate.flipTurn()
        elif player == "2" and hand_index < len(gamestate._hand2.getHand()) and placed_card_value2 < 11:
            if opening_round_filled[caravan_index+3] == False:
                opening_round_filled[caravan_index+3] = True
                gamestate.placeCard(player, hand_index, caravan_index)
                gamestate.flipTurn()
    ...
    if game_winner:
        if game_winner == "p1":
            win_status = f"Player 1 Wins!"
        elif game_winner == "p2":
            win_status = f"Player 2 Wins!"
    

    caravan_values1, caravan_values2 = gamestate.getValues()

    # Include win_status in the response
    response_data = gamestate.to_dict()
    response_data['win_status'] = win_status
    response_data['caravan_values1'] = caravan_values1
    response_data['caravan_values2'] = caravan_values2
